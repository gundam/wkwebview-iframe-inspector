...
1. `Direct` renders a Paypalweb balance page directly as the main frame, with no iframe. Its native `Safari` button presents the same balance page in Apple's `SFSafariViewController`, without the app-controlled WKWebView inspector or injected DEBUG collector.
